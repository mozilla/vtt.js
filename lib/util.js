...
function _parse(data, decoder, chunkAt, runProcModel) {
  ...
  var p = new WebVTTParser(FakeWindow, decoder),
      overlay = FakeWindow.document.createElement("div");
  p.oncue = function(cue) {
    // Also parse the cue's content and run the cue through the processing
    // model. This will result in a 'displayState' property being added to the
    // cue which is the computed div ("display state") of the subtitle.
    if (runProcModel)
      WebVTTParser.processCues(FakeWindow, [cue], overlay, null);
...
function _runProcessingModel(vttFilename, usePathUnchanged) {
  ...
  var vttData = _parse(fs.readFileSync(vttFilename).toUint8Array(), null, null,
                       false),
      overlay = FakeWindow.document.createElement("div");
  WebVTTParser.processCues(FakeWindow, vttData.cues, overlay, vttData.regions);
  return overlay;
}
...
assert.checkProcessingModel = function(vttFilename, jsonFilename, message) {
  var overlay = _runProcessingModel(vttFilename),
      vtt = JSON.parse(JSON.stringify(overlay, filterJson)),
